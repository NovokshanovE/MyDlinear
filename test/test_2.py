    ...
    def train(self, data, learning_rate, num_epochs):
        ...
        for epoch in range(num_epochs):
            for i in range(len(data) - self.window_size):
                ...
                # Повторяем y_true в каждой подпоследовательности
                y_true = y_true.unsqueeze(0).unsqueeze(-1).expand_as(x_trend)
...
input_size = 3  # Пример количества признаков во входных данных
seasonality_size = trend_size = 3  # Пример размеров слоев сезонности и тренда
window_size = 20  # Пример размера окна
...
data = torch.rand((100, input_size + 1))  # 10 временных шагов, k признаков + 1 для времени
